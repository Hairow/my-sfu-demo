...
- 信令 WS 只在**加入/离开**时有消息（`join` / `members` / `member-joined` / `member-left`），会议进行中长时间零消息 → DO 自然进入休眠
- **休眠不影响会议**：媒体流走浏览器 ↔ SFU 直连，不经过 DO，休眠时视频/音频照常
- **无损恢复**：休眠期间连接由平台托管、成员数据已 `serializeAttachment` 持久化；新成员加入触发 `fetch` 唤醒 DO，`getWebSockets()` + `deserializeAttachment()` 恢复全部在线成员，新成员拿到完整列表，老成员收到 `member-joined`
- **无需心跳保活**：不要为"保持活跃"加心跳消息，反而会频繁唤醒 DO 浪费配额；休眠不丢数据，正是附件持久化设计的原因
...
| DO 休眠/重启 | Hibernation：连接与附件数据由平台恢复，成员列表不丢 |
| 成员离开 | `webSocketClose` 广播 `member-left`，前端移除卡片、停止 track |
| 订阅失败 | 队列 `catch` 中移除成员并打日志，不影响其他订阅 |

> **DO 休眠不是故障**：数分钟无信令消息后休眠是正常状态，媒体面不经 DO 不受影响，唤醒后无损恢复（详见第 4 节"休眠机制"）。前端无需心跳保活。
